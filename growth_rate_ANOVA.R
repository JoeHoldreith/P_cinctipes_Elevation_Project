...
#make a plot of developmental stage on the x axis and growth rate on the y axis
ggplot(test_data, aes(x = yolk_embryo_ratio, y = Estimate, color = Elevation.x)) + geom_point() + scale_x_reverse() + geom_smooth(method = "lm") + labs(color = "Elevation") + scale_color_manual(values=c("red", "blue"))+xlab("Yolk/Embryo Ratio at Week 1") +  ylab("Growth Rate (mm/day)") + ggtitle("Developmental Stage and Growth Rates")+ theme(plot.title = element_text(face="bold"))

#make a plot of embryo area on x axis and growth rate on y
ggplot(test_data, aes(x = embryo_area_mm, y = Estimate, color = Elevation.x)) + geom_point() +scale_x_reverse() + geom_smooth(method = "lm") + labs(color = "Elevation") + scale_color_manual(values=c("red", "blue"))+xlab("Embryo Area (mm) at Week 1") +  ylab("Growth Rate (mm/day)") + ggtitle("Embryo Area and Growth Rates")+ theme(plot.title = element_text(face="bold"))
...
q+geom_boxplot(aes(fill = Treatment)) + scale_fill_manual(values=c("lightblue", "red")) + ylab("Growth Rate (mm/day)") + ggtitle("Growth Rates Across\nElevations and Treatments") + theme(plot.title = element_text(face="bold"))
