...
data=read.csv("6_12HSPLATE6_16RESPIROMETRY.csv")
data=data[1:61,]
...
data_zeros = data[,c(51,63,75)]
data_zeros$mean = rowMeans(data_zeros[,1:3])

data_blanks = data[,c(3,15)]
data_blanks$mean = rowMeans(data_blanks[,1:2])
...
data_corr_tall = filter(data_corr_tall, Well != "C1")
data_corr_tall = filter(data_corr_tall, Well != "D1")
data_corr_tall = filter(data_corr_tall, Well != "E2")
data_corr_tall = filter(data_corr_tall, Well != "F2")
data_corr_tall = filter(data_corr_tall, Well != "G2")
data_corr_tall = filter(data_corr_tall, Well != "G3")
data_corr_tall = filter(data_corr_tall, Well != "H2")
data_corr_tall = filter(data_corr_tall, Well != "G5")
...
#remake %O2 plots with corrected time values for all wells
...
Summarized_data = filter(Summarized_data, Well != "D6")
Summarized_data = filter(Summarized_data, Well != "G8")
Summarized_data = filter(Summarized_data, Well != "H4")
Summarized_data = filter(Summarized_data, Well != "H9")

#trim wells with dead embryos
Summarized_data = filter(Summarized_data, Well != "D7")
...
Summarized_data$Mom[1:2] = "Higher Zone Summer 9"
Summarized_data$Mom[3:13] = "Higher Zone Summer 11"
Summarized_data$Mom[14:24] = "Higher Zone Summer 12"
Summarized_data$Mom[25:29] = "Higher Zone Summer 13"
Summarized_data$Mom[30:38] = "Lower Zone Summer 2"
Summarized_data$Mom[39:48] = "Lower Zone Summer 3"
Summarized_data$Mom[49:55] = "Lower Zone Summer 5"
Summarized_data$Mom[56:63] = "Lower Zone Summer 7"
Summarized_data$Mom_numeric[1:2] = 1
Summarized_data$Mom_numeric[3:13] = 2
Summarized_data$Mom_numeric[14:24] = 3
Summarized_data$Mom_numeric[25:29] = 4
Summarized_data$Mom_numeric[30:38] = 13
Summarized_data$Mom_numeric[39:48] = 14
Summarized_data$Mom_numeric[49:55] = 15
Summarized_data$Mom_numeric[56:63] = 16
Summarized_data$Elevation[1:29] = "Higher"
Summarized_data$Elevation[30:63] = "Lower"
Summarized_data$Treatment[1:63] = "Heat Shock"
Summarized_data$Day_of_Exp[1:63] = "5"
Summarized_data$Julien_Date[1:63] = "167"
Summarized_data$Coll_date[1:63] = "Summer"

write.csv(Summarized_data,"C:/Users/joemh/Desktop/Tecan CSV files/QCd Resp Rates/6_12HS6_16respiration_rates_QC_Complete.CSV")
