...
HZ11_data = HZ11_data |> filter(X.1 != 293)
HZ12_data = data |> filter(mom %in% c("HZ12"))
HZ13_data = data |> filter(mom %in% c("HZ13"))
HZ13_data = HZ13_data |> filter(X.1 != 411)
HZ9_data = data |> filter(mom %in% c("HZ9"))
#inaccurate ellipse split, re-measured manually
HZ9_data[41,5] = 2952105
...
LZ3_data = LZ3_data |> filter(X.1 != 446)
LZ3_data = LZ3_data |> filter(X.1 != 350)
LZ5_data = data |> filter(mom %in% c("LZ5"))
LZ5_data = LZ5_data |> filter(X.1 != 266)
LZ5_data = LZ5_data |> filter(X.1 != 2458)
...
#old inaccurate version of slope extraction
#yolk_consumption_slopes = function(mom_data){
  #return(coef(lm(mom_data$yolk_embryo_ratio ~ mom_data$day_of_exp))[2])
#}
...
write.csv(all_yolk_consumption_rates, "HS_yolk_consumption_rates.csv")
